    ...
    class Program
    {
        ...
        static long Part1(string[] lines)
        {
            ...
            for (int i = 0; i < 1000; i++)
            {
                ...
                //Console.WriteLine($"{distanceList[i].Box1},{distanceList[i].Box2}, circuits : {listOfUnions.Count()}, Dist : {distanceList[i].Box1.DistanceTo(distanceList[i].Box2)}");
            }
            listOfUnions = listOfUnions.OrderByDescending(union => union.Count).ToList();
            //Console.WriteLine($"Circuits: {listOfUnions[0].Count()}, {listOfUnions[1].Count()},{listOfUnions[2].Count()},{listOfUnions[3].Count()}{listOfUnions[0][0]}, {listOfUnions[0][1]},{listOfUnions[0][2]} ");
            // Console.WriteLine(listOfUnions[0].Count());
            long sum = listOfUnions[0].Count() * listOfUnions[1].Count() * listOfUnions[2].Count();
            //Console.WriteLine(distanceList.Count());
            //Console.WriteLine($"{distanceList[0].Dist}, {distanceList[0].Box1}, {distanceList[0].Box2}");
            //Console.WriteLine($"{distanceList[1].Dist}, {distanceList[1].Box1}, {distanceList[1].Box2}");
            return sum;
            //return 0;
        }
